...
const NB_RESTAURANTS = 200;
const NB_TYPES = 10;
const NB_CITIES = 30; // pas trop de villes sinon ça flood l'api de météo !
...
async function insertManagers(managers) {
  ...
  const queryStr = `
             ...
             VALUES
             (
                 'Yann', 'Guilloux', 'yann@oclock.io',
                 '$2b$10$h4Dh2fRGAf4YdC.Cqg1yleq41QHmG61B76THHCp03SgMEizvZlscy'
             ),-- superpass
             (
                 'Nicolas', 'Charpin', 'nicolas.charpin@oclock.io',
                 '$2b$10$h4Dh2fRGAf4YdC.Cqg1yleq41QHmG61B76THHCp03SgMEizvZlscy'
             ), -- superpass
             (
              'Benjamin', 'Nougadère', 'benjamin.nougadere@oclock.io',
              '$2b$10$h4Dh2fRGAf4YdC.Cqg1yleq41QHmG61B76THHCp03SgMEizvZlscy'
             ), -- superpass
             (
              'Benoît', 'Schiex', 'benoit@oclock.io',
              '$2b$10$h4Dh2fRGAf4YdC.Cqg1yleq41QHmG61B76THHCp03SgMEizvZlscy'
             ), -- superpass
             ${managerValues}
             RETURNING id
...
function generateCities(nbCities) {
  ...
  for (let i = 0; i < nbCities; i += 1) {
    ...
    */
    ...
    const d1 = faker.datatype.number({ min: 0, max: 9 });
    const d2 = faker.datatype.number({ min: 0, max: 9 });
    const d3 = faker.datatype.number({ min: 0, max: 9 });
    const d4 = faker.datatype.number({ min: 0, max: 9 });
...
async function generateRestaurant(nbResto, managerIds, cityIds) {
  ...
  for (let i = 0; i < nbResto; i += 1) {
    ...
    const restaurant = {
      ...
      manager_id: managerIds[faker.datatype.number({ min: 1, max: managerIds.length - 1 })],
      city_id: cityIds[faker.datatype.number({ min: 1, max: cityIds.length - 1 })],
    };
...
function generateRestaurantHasCS(restaurantIds, cookingStyleIds) {
  const restaurantHasCookingStyles = restaurantIds.map((restaurantId) => {
    ...
    const nbRestaurantCookingStyle = faker.datatype.number({ min: 1, max: 3 });
    const cookingStyles = [];
